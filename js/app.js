 ...
 */
    ...
    (() => {
        ...
        const sendPost = (url, data, headers = {}) => new Promise(((resolve, reject) => {
            const xhr = new XMLHttpRequest;
            xhr.addEventListener("load", (({target}) => {
                const responseStatus = new EmailJSResponseStatus(target);
                if (200 === responseStatus.status || "OK" === responseStatus.text) resolve(responseStatus); else reject(responseStatus);
            }));
        ...
        const findHTMLForm = form => {
            let currentForm;
            if ("string" === typeof form) currentForm = document.querySelector(form); else currentForm = form;
            if (!currentForm || "FORM" !== currentForm.nodeName) throw "The 3rd parameter is expected to be the HTML form element or the style selector of form";
            return currentForm;
        };
        ...
        function formSubmit() {
            ...
            async function formSubmitAction(form, e) {
                ...
                if (0 === error) {
                    ...
                    if (ajax) {
                        ...
                        try {
                            await es.send("service_3xk13vw", "template_11f495o", {
                                from_name: formName.value,
                                to_name: "Denys",
                                from_email: formEmail.value,
                                to_email: "denikquadro@gmail.com",
                                message: formMessage.value,
                                phone_number: formTel.value
                            }, "FJi-A_-ygpzoNxqG8").then((() => {
            ...
            function formSent(form, formError) {
                ...
                if (formError) setTimeout((() => {
                    if (flsModules.popup) {
                        const popup = form.dataset.popupMessageError;
        ...
        if (inputMasks.length) inputMasks.forEach((inputMask => inputMask.addEventListener("input", (e => {
        ...
        function initSliders() {
            if (document.querySelector(".main-block__item-left") && document.querySelector(".main-block__item-right") && document.querySelector(".feedback__item-right") && document.querySelector(".item-sale-hit-swiper")) {
                let sliderLeft = new core(".main-block__item-left", {
                    modules: [ Pagination, EffectFade, Controller, Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    spaceBetween: 0,
                ...
                let sliderRight = new core(".main-block__item-right", {
            ...
            } else if (document.querySelector(".item-product-slider")) {
                new core(".item-product-slider", {
        ...
        if (player) {
            ...
            function togglePlay() {
                ...
                video[method]();
                if (video.play && isMobile.any()) controls.style.transform = "translateY(100%)";
                if (video.paused && isMobile.any()) controls.style.transform = "translateY(0)";
            }
            ...
            function toggleFullScreenMode() {
                ...
                } else if (document.exitFullscreen) document.exitFullscreen(); else if (document.mozCancelFullScreen) document.mozCancelFullScreen(); else if (document.webkitExitFullscreen) document.webkitExitFullscreen(); else if (document.msExitFullscreen) document.msExitFullscreen();
            }
        ...
        if (values.length > 0) {
            function handleClick(e) {
                const targetValue = e.target.closest(".value");
                if (!targetValue.classList.contains("value_chosen")) {
                    const chosenTargetValue = targetValue.closest(".value-wrapper__values").querySelector(".value_chosen");
                    if (chosenTargetValue) chosenTargetValue.classList.remove("value_chosen");
                    targetValue.classList.add("value_chosen");
                }
            }
            values.forEach((value => value.addEventListener("click", handleClick)));
        }
        var players = document.querySelectorAll(".player-photo-article-blog__player");
        if (players.length > 0) {
            var loadPlayer = function(event) {
                ...
                iframe.src = "https://www.youtube.com/embed/" + target.dataset.videoId + "?autoplay=1" + "showinfo=0&modestbranding=1&rel=0";
        ...
        const inputElements = document.querySelectorAll(".checkboxes-item-aside-sorting__checkbox_range");
        ...
        function documentActions(e) {
            const targetElement = e.target;
            const time = 500;
            if (targetElement.closest(".header-sorting__filter")) {
                document.documentElement.classList.add("filter-open");
                document.documentElement.classList.add("lock");
            } else if (targetElement.closest(".naming-aside-sorting__icon")) {
                document.documentElement.classList.remove("filter-open");
                document.documentElement.classList.remove("lock");
            }
            if (targetElement.closest(".item-control__image-box_cart-scrolled")) {
                document.documentElement.classList.add("cart-open");
                document.documentElement.classList.add("lock");
            } else if (targetElement.closest(".cart-header__icon")) {
                document.documentElement.classList.remove("cart-open");
                document.documentElement.classList.remove("lock");
            }
            if (targetElement.closest(".phone-header__copy")) {
                const phoneLink = document.querySelector(".phone-header__link");
                copyTextToClipboard(phoneLink);
                phoneLink.classList.add("animated");
                setTimeout((() => phoneLink.classList.remove("animated")), time);
            }
            if (targetElement.closest(".phone-copy")) {
                const burgerLink = document.querySelector(".burger-menu__link");
                copyTextToClipboard(burgerLink);
                burgerLink.classList.add("animated");
                setTimeout((() => burgerLink.classList.remove("animated")), time);
            }
        }
